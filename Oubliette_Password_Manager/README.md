...
```
    ...
    john oubliette-blowfish-complex.hash
    john oubliette-blowfish-complex-withdata-v15.hash
    john oubliette-idea-complex.hash
    john oubliette-idea-complex-withdata-v15.hash
```
The complex test vector is originally made in ISO-8859-1 and thus `--target-encoding=iso-8859-1` would be needed. In order to avoid that, the hint file `oubliette-passwords.txt` now contains two versions of it, one in the original UTF-8 and one in ISO-8859-1, so that extra option is not needed.
